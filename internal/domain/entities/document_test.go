...
func TestDocument_Clear(t *testing.T) {
	...
	content, err := valueobjects.NewContent("some content")
	if err != nil {
		t.Fatalf("Failed to create content: %v", err)
	}
	if err := doc.UpdateContent(content); err != nil {
		t.Fatalf("Failed to update content: %v", err)
	}
...
func TestDocument_GetAttachments(t *testing.T) {
	...
	fileName1, err := valueobjects.NewFileName("test1.txt")
	if err != nil {
		t.Fatalf("Failed to create fileName1: %v", err)
	}
	fileName2, err := valueobjects.NewFileName("test2.txt")
	if err != nil {
